...
*
* Amiga interface amiga.c by David Kinder
*
\*************************************************************************/
...
char *amiga_getgfxname(char *gamename);
...
type8 *picdata;
  ...
  switch (mode)
  {
    ...
    case 2:	/* Graphics on (normal) */
      ...
      current_pic = -1;
      amiga_freepic();
      picdata = ms_extract(c,&pic_width,&pic_height,pic_palette);
      if (picdata)
      {
	current_pic = c;
...
type32 dlimit, slimit;
  ...
  if (gfxname == 0) gfxname = amiga_getgfxname(gamename);
...
char Version[] = "$VER:Magnetic 1.0 (29.8.97)";
...
int i, window_top;
  ...
  if ((Window = OpenWindowTags(0,
    ...
    WA_Height,Screen ? Screen->Height-window_top : ScreenHeight-window_top,
    WA_SmartRefresh,1,
    WA_NewLookMenus,1,
    WA_AutoAdjust,1,
...
void amiga_about(void)
{
  amiga_req("Magnetic Scrolls Interpreter 1.0\n\n"
	    "Written by Niclas Karlsson\n"
	    "Amiga front-end by David Kinder","Continue");
}
...
char *extpart;
  ...
  filepart = FilePart(gfxname);

  extpart = strrchr(filepart,'.');
  if (extpart == NULL) extpart = filepart+strlen(filepart);
  strcpy(extpart,".gfx");

...
*
*
* Magnetic v2.0, 32 bit DOS Allegro version.
*
* Allegro 32 bit DOS DJGPP interface by David Kinder, based in
* part on Stefan Jokisch's 16 bit DOS interface.
*
* After building, Magnetic should be set up to use the WDOSX DOS
* extender, by running a command of the form
*
*    \wdosx\bin\stubit magnetic.exe
*
\***********************************************************************/
...
void process_args(int argc, char** argv)
{
  ...
  while (i < argc)
  {
    ...
    else if (got_story == 0)
    {
      ...
      set_filename(music_name,argv[i],".mp3");
  ...
  if (got_story == 0)
  {
    puts(
      ...
      "Copyright (C) Niclas Karlsson 1997-2000.\n"
      "Written by Niclas Karlsson, David Kinder, Stefan Meier and Paul David Doherty.\n\n"
      "Syntax: magnetic [options] story-file [graphics-file]\n\n"
...
void hotkey_info(void)
{
  ...
  screen_string("\nMagnetic v2.0, an interpreter for Magnetic Scrolls games.\n");
  screen_string("Copyright (C) 1997-2000 Niclas Karlsson.\n\n");
...
void title(void)
{
  ...
  /* Check for the presence of the MP3 file. */
  if (access(music_name,F_OK) != 0)
    use_sound = 0;
  ...
  if (use_sound)
  {
    ...
    /* If there's no music, just wait for a key. */
    if (music == FALSE)
      readkey();
  }
  else
...
int main(int argc, char** argv)
{
  ...
  init_code = ms_init(story_name,gfx_name);
  if (init_code == 0)
...
type8 ms_getchar(void)
{
  ...
  if (input_buffer[0] == 0)
  {
    if (input_line(input_buffer,INPUT_LIMIT,TRUE) == INPUT_UNDO)
      return 0;
    input_index = 0;
  }

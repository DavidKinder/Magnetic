...
This is a "port" of the Magnetic Scrolls 2.0 interpreter to Glk.  The
...
Typically, this will be a game with the extension ".MAG" (and "MaSc" in the
first four data bytes of the file).
...
extension ".GFX", then make sure that it is in the same directory as the
main data file, and has the same base name.  For example, the matching
...
will run GUILD.MAG, and also use pictures from GUILD.GFX if available.
...
that any associated graphics are held in files with ".GFX" or ".gfx"
extensions.  Games held in files with other extensions may not work.

There are a few command line options that you can also add, to vary the way
that the game looks to a small extent:
...
To build the IFP plugin, use
	...
	make -f Makefile.glk magnetic-2.0.so
...
timers, if there is no suitable .GFX graphics file, or if the interpreter
cannot open the graphics file successfully.  See below for further notes
about displaying pictures.

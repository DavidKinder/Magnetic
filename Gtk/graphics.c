...
static gint current_picture = -1;
static gint current_mode = -1;
...
void graphics_refresh ()
{
    ...
    if (current_picture != -1 && current_mode != -1)
	ms_showpic (current_picture, current_mode);

    if (config.graphics_bg != NULL)
    {
	GdkColor colour;

	/*
	 * The picture's parent widget is, I believe, an automagically
	 * created viewport thingy, so we don't have any handle to it.
	 */
	if (gdk_color_parse (config.graphics_bg, &colour))
	    gtk_widget_modify_bg (gtk_widget_get_parent (GUI.picture),
...
static void free_picture (struct picture *picture)
{
    ...
    {
	gtk_timeout_remove (picture->animation_timer);
	picture->animation_timer = 0;
    }
    if (picture->rgb_data != NULL)
    {
    ...
    if (picture->anti_frames != NULL)
    {
	gint i;

	for (i = 0; i < picture->anti_frame_count; i++)
	    if (picture->anti_frames[i].data != NULL)
...
void ms_showpic (type32 c, type8 mode)
{
	...
	*ptr++ = picture.rgb_palette[raw_data[i]].blue;
	...
	if (display_animation (&picture) && config.animate_images)
		...
		gtk_timeout_add (config.animation_delay, display_animation,
				 &picture);

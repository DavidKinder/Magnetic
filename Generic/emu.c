...
*
* Copyright (C) 1997-2002  Niclas Karlsson <nkarlsso@ra.abo.fi>
*
...
*
* 2.1  060902 : [SM] Replaced non-ANSI strupr with new routine (contributed
...
static type8s undo_ok[] = "\n[Previous turn undone.]";
static type8s undo_fail[] = "\n[You can't \"undo\" what hasn't been done!]";
static type8s no_hints[] = "[Hints are not available.]\n";
static type8s not_supported[] = "[This function is not (yet) supported.]\n";
...
type8 hintsAvail = 0;
struct ms_hint hints[MAX_HINTS];
type8s* a0df_workaround = 0;
...
type8 ms_init(type8s * name, type8s * gfxname, type8s * hntname)
{
	...
	if (version == 4) {
		...
		if (hntname && (hnt_fp = fopen(hntname, "rb"))) {
		   ...
		   {
              type8 buf[8];
			  type16 i,j,blkcnt,elcnt,ntype,elsize;
			  // read no of blocks
			  ...
			  for (i=0; i < blkcnt; i++) {
			     ...
			     for (j=0; j < elcnt; j++) {
				    fread(&buf, 1, 2, hnt_fp);
			        elsize = read_w2(buf);
				    fread(&hints[i].content[j], 1, elsize, hnt_fp);
					hints[i].content[j][elsize-1]= '\0';
#ifdef LOGHNT
			        out2("%s\n",hints[i].content[j]);
#endif
				 }
...
void do_line_a(void)
{
	type8 l1c;
	type8s *str, *msg;
	type16 ptr, ptr2, tmp16, d2reg, d0reg;
	type32 a1reg, tmp32;
	...
	if ((byte2 < 0xdd) || (version < 4 && byte2 < 0xe4) || (version < 2 && byte2 < 0xed))
	{
		...
		else
		{
			if ((l1c = ms_undo()) != 0)
				...
				str = undo_fail;
			for (tmp16 = 0; str[tmp16]; tmp16++)
				ms_putchar(str[tmp16]);
			if (!l1c)
				write_reg(1, 2, '\n');
		}
	}
		...
		case 2:	/* A0DF */
			...
			if ((d0reg == 2) && (version == 4) && (ioEnabled)) {
				if (strcmp(ms_strupr(a0df_workaround),"HINTS") == 0) { 
				   if (hintsAvail) {
				   	  type8 hres = ms_showhints(hints);
					  if (!hres) {
			             for (tmp16 = 0; no_hints[tmp16]; tmp16++)
				            ms_putchar(no_hints[tmp16]);
					  }
				   } else {
				  	  type16 tmp16 = 0;
			          for (tmp16 = 0; no_hints[tmp16]; tmp16++)
				...
				} else if (strcmp(ms_strupr(a0df_workaround),"MAP") == 0) {
				  	  type16 tmp16 = 0;
			          for (tmp16 = 0; not_supported[tmp16]; tmp16++)
				         ms_putchar(not_supported[tmp16]);
				} else if (strcmp(ms_strupr(a0df_workaround),"COMPASS") == 0) {
				  	  type16 tmp16 = 0;
			          for (tmp16 = 0; not_supported[tmp16]; tmp16++)
				         ms_putchar(not_supported[tmp16]);
				}
			} else {
			   if (gfxtable == 0 && table_dist == 0)
			   {
			  	  table_dist = findGfxTable();
			...
			do
			{
				if (!(l1c = ms_getchar()))
				{
					if ((l1c = ms_undo()) != 0)
						...
						msg = undo_fail;
					for (tmp16 = 0; msg[tmp16]; tmp16++)
						ms_putchar(msg[tmp16]);
					if (!l1c)
					{
						tmp16 = 0;
						str[tmp16++] = '\n';
